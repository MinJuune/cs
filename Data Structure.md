...
**완전 이진 트리(complete binary tree)**는 마지막 레벨을 제외한, 모든 노드들이 채워진 형태,    
**포화 이진 트리(full binary tree)**는 모든 노드가 0개 또는 2개의 자식을 가지는 형태,    
**편향 이진 트리(skewed binary tree)**는 한쪽으로만 치우친 형태입니다.     
